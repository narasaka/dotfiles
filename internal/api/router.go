...
func NewServer(deps ServerDeps) *Server {
	...
	s := &Server{
		...
		clusterHandler:  NewClusterHandler(deps.Clusters),
		webFS:           deps.WebFS,
	}
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// Cluster Plugins
			r.Get("/providers", s.clusterHandler.ListProviders)
			r.Post("/providers/{provider}/validate", s.clusterHandler.ValidateCredentials)
			r.Post("/providers/{provider}/discover", s.clusterHandler.DiscoverClusters)

			// Clusters
			r.Get("/clusters", s.clusterHandler.List)
			r.Post("/clusters", s.clusterHandler.Connect)
			r.Get("/clusters/{id}", s.clusterHandler.Get)
			r.Delete("/clusters/{id}", s.clusterHandler.Delete)
			r.Post("/clusters/{id}/sync", s.clusterHandler.Sync)
			r.Get("/clusters/{id}/metrics", s.clusterHandler.GetMetrics)
			r.Get("/clusters/{id}/events", s.clusterHandler.ListEvents)

			// Node Pools
			r.Get("/clusters/{id}/node-pools", s.clusterHandler.ListNodePools)
			r.Post("/clusters/{id}/node-pools", s.clusterHandler.CreateNodePool)
			r.Put("/clusters/{id}/node-pools/{poolId}", s.clusterHandler.ScaleNodePool)
			r.Delete("/clusters/{id}/node-pools/{poolId}", s.clusterHandler.DeleteNodePool)
			r.Get("/clusters/{id}/node-pools/{poolId}/nodes", s.clusterHandler.ListNodes)
		})
